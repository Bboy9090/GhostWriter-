...
| **Mobile vs desktop**     | **Desktop main page**: screenshot OCR + manual paste + optional cloud search (no fake auto-ingest). **Mobile**: same web app; use **iOS Upload** in the dev drawer for batch images/video frames. **Extension** = side panel capture (see `extension/README.md`). |

## Prioritized implementation plan (completed in this pass)

1. **Capture / upload** — Real OCR on image drop; clear video vs image routing; no simulated vault feed.
...
- [x] **Floating portal** toggle → widget styling only (not OS screen capture in the browser).
...
| Phase | Files / areas                                                                                                                    |
| ----- | -------------------------------------------------------------------------------------------------------------------------------- |
| 1     | `src/App.tsx` (dropzone, portal widget), `src/lib/ocr-browser.ts`                                                                |
